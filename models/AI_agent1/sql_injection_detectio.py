...
# def run_sql_model():
#     file_path = "models/AI_agent1/advanced_generated_sql_injections.csv"
#     data = pd.read_csv(file_path)

#     def detect_sql(payload):
#         if "OR 1=1" in payload or "DROP TABLE" in payload or "UNION" in payload:
...
#     # Return first result for demo
#     return data["Result"].iloc[5]
...
def run_sql_model():
    ...
    # Return multiple results (better demo)
    results = data[["Payload", "Result"]].head(5)

    return data[["Payload", "Result"]].head(10).to_dict(orient="records")